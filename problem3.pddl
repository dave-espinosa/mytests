...
(:init
    ...
    (amb_free A1)           ;Ambulancia A1 está libre
    (amb_free A2)           ;Ambulancia A1 está libre
    (amb_on_loc A1 L1)      ;Ambulancia A1 ubicada en L1
    (amb_on_loc A2 L1)      ;Ambulancia A2 ubicada en L1
    (pac_on_loc P1 L4)      ;Paciente P1 ubicado en L4
    (pac_on_loc P2 L3)      ;Paciente P2 ubicado en L3
    (not(pac_on_amb P1 A1)) ;Paciente P1 no está ubicado en A1
    (not(pac_on_amb P2 A1)) ;Paciente P2 no está ubicado en A1
    (not(pac_on_amb P1 A2)) ;Paciente P1 no está ubicado en A2
    (not(pac_on_amb P2 A2)) ;Paciente P2 no está ubicado en A2
...
(:goal (and
    ...
    (amb_on_loc A1 L1) ;Ambulancia A1 en L1 (Hospital)
    (amb_on_loc A2 L1) ;Ambulancia A2 en L1 (Hospital)
))
